...
Last refined: 2026-05-08 (v3 — adversarial pass).

This is the canonical working backlog for AI strength research.

## End Goal: Working RL Loop

The destination is a **working DAgger iteration loop** (Option A): the planner from item 2 relabels states the model actually visits, the model retrains on the mixed dataset, and a per-iteration gate decides whether to promote or roll back. Online PPO (Option B, see F1) and AlphaZero-style search-at-training-time (Option C, see F2) are gated follow-ups, not parallel tracks.

Pure supervised learning is bounded by teacher quality. Corrected trained policies are around 34-40%, corrected rollout is 42.5%, corrected depth-2/top-12 search is 37.5%; even with perfect features and large datasets the imitation ceiling is in the 55-65% range. RL self-play has no such cap and reaches 80-95% against a fixed rule bot in this kind of game.
...
### 0. Measure Current Sim And Planner Throughput Before Sizing The Loop
...
### 2. Bring The DAgger Teacher Above The Gate
...
- Tune bounded turn-bundle expansion across trainer, evolve, attach, ability, retreat, and combat phases. State explicitly the maximum bundle depth tried (e.g., depth 3 same-turn → depth 4 with one opponent response).
- Combine planner leaves with common-random rollout/search scoring; record per-lever lift (bundle expansion, CRN scoring, ranker diversity) so the 65% target is decomposed, not asserted.
- Use ranker diversity and exhaustive small-state audits to keep dropped-best rate under 2%.
- Pre-register an escalation rule: if planner stalls below 55% Wilson lower bound after the listed work and a 2-week effort budget, trigger plan B — fall back to a stronger teacher composition (planner + value-head tiebreaker once item 7 lands; or rollout-augmented planner with deeper CRN). Do not silently lower the gate.
...
### 4. Ship The DAgger Trace-To-Training Data Recipe
...
### 5. Versioned Feature Migration For Card Identity And Context
...
### 6. Parallelize Generation For Self-Play Throughput And Add Matchup Sampling
...
### 8. Make Training Scale-Ready Before Large Sweeps
...
### 7. Calibrate Value/Action-Value As The RL Critic
...
### 9. Harden The Procedural Scorers The Planner Consumes
...
### 11. Build A DAgger Iteration Orchestrator
...
### 12. Maintain An Opponent Snapshot Pool
...
### 13. Per-Iteration Eval Gate With Regression Rollback
...
### 14. Compute Budget And Distillation Path
...
### 15. Exploration During Trace Generation And Action Coverage
...
### 16. Iteration-Level Debugging Tooling
...
### F1. Online PPO Plumbing (Gated On Item 13 Producing A Strong Policy)
...
### F2. AlphaZero-Style Search-At-Training-Time (Gated On F1)
