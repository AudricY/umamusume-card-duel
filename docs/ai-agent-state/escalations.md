...
- **2026-05-15 MCTS-distill v1 closeout (FAIL).** Iter-0 SL gate Wilson lower **0.1470** (n=1000 side-balanced, rule-bot, seed-start 9000), point 0.169, on `runs/mcts-distill-v1/iter-000/`. <0.30 → scoping § 6 FAIL + § 5(c): **state-coverage mismatch binding** (mcts-vs-mcts corpus vs raw-policy rule-bot gate) — the pre-registered biggest risk. Soft-visit label shape is the worst F1 iter-0 axis (-15 to -18pp vs R7/R8/R7.b.2). 4 F1 axes exhausted (labels/objective/repr/label-shape). MCTS-distill SL branch closed. Repro-from-item17 blocked by post-R7.b.2 arch incompat (run README); canonical gate verdict intact (commit 73c2d8c). **Next: queue `mcts-distill-followup-direction` P2 — change warm-start (W6/iter-2) or supervision distribution; escalate pick to user.** Detail: `progress/r15.md` § MCTS-distill v1.
