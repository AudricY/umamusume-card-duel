...
**Outcome:** all code-side workstreams landed. Headline strength **iter-2 Wilson 0.6479 at rollout-leaf MCTS** (I.2). Cheap-inference (value-head leaf) on iter-2: **Wilson 0.452 at adaptive-ratio=1.5** (F-iter-2 production pick), **2.56× speedup vs ratio=0**; OOD-robust via A. The earlier A datapoint (Wilson 0.404 at seeds 800000+) and the F-iter-2 datapoint (Wilson 0.443 at seeds 820000+) suggest the side asymmetry from A is partly seed-clustered. Engine determinism (B) closes R-WILD #34. UI plumbing (E) verified end-to-end via headless smoke (decisionMs 200 at 16 sims → ~0.5s at 100 sims with ratio=1.5); only the 20-game in-browser exercise is left. ORT unpinning (G) showed 1.05× speedup at this model size — kept `--ort-threads 1`. D (PPO V-trace) and H (predict-batching) skipped per the plan's decision points.
