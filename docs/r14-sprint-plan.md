...
**Outcome:** all code-side workstreams landed. Headline strength **iter-2 Wilson 0.6479 at rollout-leaf MCTS** (I.2). Cheap-inference (value-head leaf) on iter-2: F-iter-2 reported **Wilson 0.452 at adaptive-ratio=1.5** (production pick at the F seed range), **2.56× speedup vs ratio=0**. **Update 2026-05-14 (A.footnote):** a third-seed-range gate at seeds 900000+ failed the 0.40 bar at **Wilson 0.394**. Across F-iter-2 (0.443 at seeds 820000+) + A (0.404 at seeds 800000+) + A.footnote (0.394 at seeds 900000+), the empirical range is **Wilson lower 0.39–0.45**; the 0.452 number should be cited as the upper end of that range, not a stable point estimate. The cheap-inference deployment claim is materially weakened; the rollout-leaf iter-2 production pick (Wilson 0.6479) is untouched by this gate. The A.footnote also refuted A's "side asymmetry is partly seed-clustered" — the player/opponent gap (+0.11–0.30pp Wilson) replicates across all four gates and two independent seed ranges. Engine determinism (B) closes R-WILD #34. UI plumbing (E) verified end-to-end via headless smoke (decisionMs 200 at 16 sims → ~0.5s at 100 sims with ratio=1.5); only the 20-game in-browser exercise is left. ORT unpinning (G) showed 1.05× speedup at this model size — kept `--ort-threads 1`. D (PPO V-trace) and H (predict-batching) skipped per the plan's decision points.
...
- **F — DONE (production pick = ratio 1.5 on iter-2; 2026-05-14).** Ran twice. First on iter-1 (the original sprint target): stated FAIL (0.42 Wilson unreachable at fresh seeds), but ratio=2.0 Pareto-dominated baseline at 2.19× speedup. Re-ran on iter-2 (the I.2 production candidate) — **status PASS**, all 5 ratios clear 0.42 Wilson. Pareto frontier on iter-2: ratio=1.5 (Wilson 0.452, **2.56× speedup**) and ratio=5.0 (Wilson 0.472, 1.53× speedup); 2.0 strictly dominated. **Production pick: ratio=1.5** for W5/E default — Wilson 0.452 at ~0.5s/decision (vs R13.W6 iter-1's ~1.25s/decision), 2.5× speed-equivalent strength gain at the cheap-inference deployment point. Output: `runs/R14-adaptive-sweep/` (iter-1) and `runs/R14-adaptive-sweep-iter2/` (iter-2).
