# R16 — Model Feature Backlog Refinement (P0/P1 results)

Canonical home for completed R16 phase results. Scope, recipe rationale, and
the as-built P1 contract live in
`docs/ai-research/scoping/r16-model-feature-backlog-refinement.md`; this file
holds the evidence and verdicts. Other state files point here.

Cap: ≤500 lines. Roll older blocks to a sibling archive once exceeded.

---

## R16-P0 — MCTS self-play v3 embedding support — DONE

Data-path parity fix (commit `b38de0e`): MCTS-distill now exercises the v3
card-embedding branch instead of training it inert. Confirmed working by the
R110 W6 reproduction (real monotone `0.527→0.561→0.604` vs the inert
confounded run). Canonical reproduction evidence:
`docs/ai-research/progress/r110.md`.

---

## R16-P1 — Temporal / turn-state v3.1 (164-d) — IMPLEMENTED, ABLATION = NO-GO

### 1. Verdict: NO-GO — v3.1 ≈ v3.0, no Wilson-lower win, production stays 96-d

The 164-d temporal/turn-state vertical was implemented end-to-end and ablated
faithfully against the v3.0 baseline. Promote criterion: a Wilson-lower WIN of
the v3.1 best-promoted aggregate over the v3.0 R110-W6-repro best-promoted
baseline of **0.6042**.

v3.1 best-promoted is **0.5955 (iter-3)** — **0.0087 BELOW 0.6042**, within
gate noise (no gain, slight underperformance). v3.1 never clears the v3.0
peak and oscillates in a ~0.52–0.60 band. **v3.1 is NOT promoted; production
stays pinned 96-d.** The temporal/turn-state representation produced a **null
strength signal**.

### 2. Implementation status — IMPLEMENTED

- **v3.1 vertical** (commit `357f0d6`): TS observation schemaVersion 2→3
  (`temporal` block + per-side/per-Uma `turnState`, counts-only ability
  locks, derived `effectiveRetreatCostReduction`) → NEW Python
  `observation_to_features_v3_1` (164-d; frozen v3.0 builder verbatim for
  slots 0–109 byte-identical, +54 temporal slots) → serve_onnx `164→v3.1`
  schema entry → 164-d ONNX export. `STATE_DIM` stays a frozen alias of
  `STATE_DIM_V3=110`; v3.1 is opt-in via dim-keyed selectors.
- **Trainer wiring** (commit `3ce1404`): `--state-dim` threaded through
  `selfplay_dataset.py` / `train_bc.py` / `r12_orchestrator.py`, plus
  `make_v31_additive_init.py` — a 164-d additive-tail warm-start init proven
  output-identical to the v3.0 init (**logit/value delta = 0.0**), the
  cleanest possible A/B init.

As-built P1 contract and the two resolved scope omissions
(`globalRetreatCostReduction` → derived effective field; `ownIsFirstTurn` =
energy/setup-phase flag `turnsTakenBySide===0`):
`docs/ai-research/scoping/r16-model-feature-backlog-refinement.md` § P1.

### 3. Trajectory + reasoning (rolled to archive)

v3.1 6-iter Wilson-lower: 0.5189/0.5869/0.5442/**0.5955**/0.5527/0.5527
vs v3.0 R110-W6-repro best iter-2 **0.6042**: Δ=−0.0087, within-noise,
no Wilson win → **NO-GO**. Oscillates in ~0.52-0.60 band; +2-iter
extension confirmed no late climb (iters 4-5 flat at 0.5527).
Schema-independent W6 pattern (canonical: `r110.md §3` — intrinsic
W6-recipe property, not a representation effect). Full trajectory
table + reasoning rolled to `r16-archive.md` § "R16-P1 v3.1 trajectory".

### 5. Methodology / harness facts / residual gaps (rolled to archive)

- Faithful R110-W6-repro mirror (rollout leaf, 100 sims, CRN 3, 200
  rollout steps, n=120 side-balanced gate at `--eval-seed-start 9000`,
  unregularized; 164-d additive-tail init delta=0.0; faithful
  `--resume-state` for iters 4-5).
- Durable harness fact: `serve_onnx.py` TCP backlog 5→128
  (`request_queue_size=128`) under 24-worker gate fan-in; commit
  `e846881`; 0 connection errors across all subsequent runs.
- Benign residual: `training/r14_value_crossover_probe.py` is not
  wired for 164-d; logged-only diagnostic, zero effect on gated
  checkpoints.
- Two corrected recipe-confounds (process lessons): W6-fix flags
  defaulted ON (corrected with `--no-w6-fix-*`); `--eval-games 120`
  was wrong (the real gate is `--games 60`). Lesson: verify effective
  config in `events.jsonl`, never infer flag defaults; verify recipe
  args against the real baseline `gate.log`.

Full prose (methodology + recipe-confound walk-through) rolled to
`docs/ai-research/progress/r16-archive.md`.

### 8. Cross-references

- Scope / as-built P1 contract:
  `docs/ai-research/scoping/r16-model-feature-backlog-refinement.md`
- Schema-independent W6 mechanism + v3.0 baseline:
  `docs/ai-research/progress/r110.md`
- Forward-looking backlog: `docs/ai-research-backlog.md` (item 2 / 2b)

---

## R16 Training-Data — Contested-Coverage Pilot

**Verdict: POSITIVE cheap slope (direction-only).** At fixed retained-row
count and fixed capacity, raising the audit's `legal_action_count` contested
fraction (0.20 → 0.30 → 0.45) produced a **monotone, side-balanced
Wilson-lower improvement** vs the coverage-matched baseline. The
pre-registered acceptance condition (monotone side-balanced Wilson-lower rise
as contested fraction crosses the ≥30% floor) is **met** — the
contested-coverage hypothesis is *not* falsified. Magnitude is small
(~+2.2pp side-balanced Wilson-lower across the full 0.20→0.45 span at
n=300/side, within single-gate noise); the *slope sign* is the
pre-registered signal and it is consistently positive across all three
points, on both the worst-side bound and overall WR. This unlocks (does not
itself perform) a larger-n confirmation gate; it does NOT by itself license
the expensive closed-loop / data-generation spend.

### Mechanisms as built (write scope: `training/uma_ai/dataset.py`)

Two independently-selectable knobs, both holding total retained-row count
fixed, both defaulting OFF (bit-identical no-op when unset — verified):

- **Option 3 — legal-action-count loss weighting.** `contested_loss_weight`
  (×1.0 = no-op) multiplies `_sample_weight` for rows with
  `≥ contested_min_legal` (default 4) legal actions. Reuses the existing
  `sample_weight` / `collate_policy_batch` / `normalized_weights` seam; no
  corpus change, no row-count change.
- **Option 1 — contested resampling.** `contested_resample_fraction`
  (`None` = no-op) buffers the retained stream, splits on the ≥4-legal
  threshold, and draws with replacement from each group to hit the target
  contested fraction while keeping total retained count exactly fixed
  (deterministic, RNG seeded by `--seed`). Empty-group guard yields the
  unmodified stream rather than fabricating distribution.

Threaded into `train_bc.py` (bc mode only) as `--contested-loss-weight`,
`--contested-min-legal`, `--contested-resample-fraction` — same
`--state-dim`-style option threading; no new config framework.

### Sweep (Option 1, the variable swept here)

Corpus: `runs/R7-multi-teacher-warmstart/iter-000/mixed-v3.jsonl` (the
Phase-4 v3 re-extraction of the audited R7 corpus — same 3789 retained
rows; the v2 `mixed.jsonl` cannot be loaded by the current
`cardIdsByZone`-hardened loader). Train: 8 epochs, hidden-dim 64, depth 3,
seed 7, v3.0/110-d (capacity FIXED). Gate:
`backend/src/sim/evaluateModelVsHeuristic.ts`, rule-bot, side-balanced,
300 games/side (600 total — under the n≥1000 stop-rule cap), seed-start
9000.

| Coverage point | `legal_action_count` | player WR | opp WR | **side-bal Wilson-lower** | overall WR |
| --- | ---: | ---: | ---: | ---: | ---: |
| baseline (no knob) | 0.2022 | 0.290 | 0.393 | **0.2416** | 0.342 |
| resample ~30% | 0.3001 | 0.307 | 0.393 | **0.2572** | 0.350 |
| resample ~45% | 0.4500 | 0.313 | 0.397 | **0.2635** | 0.355 |

Side-balanced Wilson-lower = the worst-side Wilson-95 lower bound (the
gate's side-balanced strength signal). Monotone increase 0.2416 → 0.2572
→ 0.2635; improvement is driven by the weaker (player) side, opponent side
flat. Baseline 0.2022 reproduces the audit headline (below the ≥30% floor;
R7 retained is exactly-2-legal-dominated).

### Methodology / cross-references (rolled to archive)

Pre-registered protocol: no-op invariant → coverage-moves verified
(Option 1 hits 0.2022/0.3001/0.4500 exactly on real audited corpus
at fixed retained count) → one cheap point first → full 3-point
sweep. n≥1000 closed-loop gate NOT launched (stop rule: slope must
be positive first — which it is). Full prose, pre-registered design,
contested-metric definition, and v2/v3 loader environment note all
rolled to `r16-archive.md` § "Contested-Coverage Pilot — Methodology
+ cross-references". Pre-registered design canonical:
`scoping/r16-training-data-backlog-refinement.md` § "Fork A —
Contested-State Data Coverage".

## R16 Training-Data — Online MCTS Relabel Infra (3a emit-path)

### Corrected cost finding (reprioritization basis — archived)

Read-only sizing pass (2026-05-18) showed the prior "INFRA-BLOCKED /
heavy expensive arm" framing was wrong: MCTS already runs on the live
`GameState` at the decision point and all diagnostics + audits
already exist; only the discarded `MctsResult.visits/diagnostics`
needed wiring (~2-4 eng-day emit-path, not a serializer). 3a was
therefore reprioritized ahead of `r16-p2`. Full historical framing
rolled to `r16-archive.md`.

### What landed (3a emit-path, this chunk only)

- New `--relabel-mcts` mode + `--relabel-state-source` tag in
  `backend/src/sim/evaluateModelVsHeuristic.ts`. A sibling
  `runMctsRelabel` runs rollout-leaf, no-Dirichlet MCTS on the live
  `GameState` at the decision point and threads the full `MctsResult`
  (`visits` + `diagnostics`) into three new optional `DecisionTraceRow`
  fields: normalized `policyTargets`, an `oracle` block, and a
  `stateSource` tag. Forced states (`legalActions.length <= 1`) suppress
  row emission entirely (do not fall through unlabeled).
- `backend/src/sim/dagger/relabelDecisionTrace.ts`: a
  `--label-source rollout-leaf-mcts` pass-through branch forwards the
  evaluator-emitted `policyTargets`/`oracle` verbatim and reuses the
  existing leak / index-range / sum-to-1 / length gates (no new audit
  logic, no mixture recomputation).
- No-op invariant VERIFIED: with `--relabel-mcts` off, trace rows carry
  none of the new fields and are deterministic/bit-identical across runs;
  full `npm run test:train` (incl. `multiTeacherTraceSmoke`,
  `daggerRoundSmoke`) passes; `npm run build` clean. ~0 functional
  `mcts.ts` change (one type re-export only).

### Pilot smoke + Audit-v2 (superseded by prod corpus)

50-game rule-bot-mirror pilot (modelSide=both = 100 games, 50 sims, 14m23s
wall-clock single-thread): 2127 contested rows, Audit-v2 ALL PASS (100%
retained, oracle present, no leaks). Superseded by the 3-recipe production
corpus run below (§ "Production Corpus — Audit-v2 ALL PASS"); pilot kept
here as the cost-validation reference for the multi-worker harness spike.

### Source-Recipe Matrix

The three source recipes from scoping § P1 are achieved via the existing
`--selection` + `--relabel-state-source` flags on
`backend/src/sim/evaluateModelVsHeuristic.ts`; **no new flag is needed**.
`runMctsRelabel` operates on the live `GameState` independent of which
selection played the action, so the recipe is fully determined by the played
selection (`--selection`) plus the corpus tag (`--relabel-state-source`).

Per-recipe cost (per decision; CPU cost dominated by MCTS sims × rollout
steps):

| Recipe | `--selection` | Played-action cost | Relabel cost | Model server required? |
| --- | --- | --- | --- | --- |
| `rule-bot-mirror` | `baseline` | 0 NN, 0 MCTS | 1 MCTS call | **No** |
| `policy-vs-rule` | `policy` | 1 NN forward | 1 MCTS call | Yes |
| `search-vs-rule` / `mcts-vs-rule` | `search` or `mcts` | 1 MCTS call (played) | 1 MCTS call (relabel) | Yes if `--mcts-prior policy` (preferred — uniform priors degrade played-search quality, so `search-vs-rule` is effectively serve-required) |

Smoke landed: `backend/src/tests/relabelMctsSmoke.ts` codifies the matrix in
three sub-cases — (a) `--relabel-mcts` ON + rule-bot-mirror with full
`policyTargets` / `oracle` / `stateSource` audit + relabel pass-through; (b)
same with `policy-vs-rule` tag (proves the state-source tag wires through
independently of played selection); (c) `--relabel-mcts` OFF as the no-op
regression guard (no `policyTargets` / `oracle` / `stateSource` field leakage
when the mode is unset). Wall-clock ~2.5s; wired into `npm run test:train`.

Next user-gated chunk (intentionally not auto-launched): the 200-400-game
production corpus + Audit-v2 acceptance gate against the recipe matrix.

### Multi-Worker Corpus Harness (compute optimization spike — compressed)

`evalGate.ts` (which already implements work-stealing dispatch over
`runModelVsHeuristicGame`) extended to host the corpus generator:
parses `--relabel-mcts`/`--relabel-state-source`, owns the trace
file, buffers `result.decisionTraces` by `taskIndex`, flushes in
deterministic order at run end (verified bit-identical across N=1/16/24).
Smoke `evalGateCorpusGenSmoke.ts` locks the contract. Wall-clock on
pilot-equivalent recipe: N=1 873s → **N=16 150s (5.83×)** → N=24
154s (past the knee). Speedup sub-linear because per-game variance
× ~6 games/worker = straggler tail dominates. Full benchmark table
+ design notes rolled to `r16-archive.md`.

### Production Corpus — Audit-v2 ALL PASS

3-recipe production corpus generated 2026-05-21 (start 09:54, end
10:30, total wall-clock ~35.5 min on the 16-worker harness against
the 96-d production pin):

| Recipe | Played-side WR | Total contested rows | Per-recipe Audit-v2 |
| --- | ---: | ---: | --- |
| rule-bot-mirror | n/a (both sides rule-bot) | 4196 | PASS 100% retained |
| policy-vs-rule | **0.319** (raw policy weak vs rule-bot) | 3664 | PASS 100% retained |
| search-vs-rule | **0.490** (search lifts model to near-parity) | 3938 | PASS 100% retained |
| **AGGREGATE** | — | **11798** | **PASS 100% retained** |

`stateSource` tags match recipe; every row carries the full 11-key
`oracle` block + simplex `policyTargets`; no forced-state row
emitted; ~50% of rows ≥3 legal (the contested signal 3b consumes).
Promote criterion (scoping § P1 acceptance, retained
contested-row rate ≥80%) MET trivially.

Pre-launch served-recipe smokes (`policy-vs-rule` 16 rows /
`search-vs-rule` 28 rows, both PASS oracle+simplex+stateSource
against `runs/R13-W6-phase-d/iter-2/policy.onnx`) and the BC
loader-retention check (`JsonlPolicyDataset` accepts 100% of these
rows, exercises the soft `policyTargets` path in
`train_bc.py:490-498`, `oracle`/`stateSource`/`selectedOriginalRank`
silently ignored) rolled to `r16-archive.md` §§ "R16-TD 3a — Loader
Verification full detail" / "R16-TD 3a — Served-Recipe Smokes full
detail" / "R16-TD 3a — Production Corpus per-slice coverage".
Launch script: `runs/R16-TD-3a-prod-corpus/launch.sh` (3-phase
sequential, trap-handled server lifecycle).

Latent forward-looking risk: `dataset.py:34-43` notes
`ROW_SCHEMA_VERSION` was deliberately held at 1 across Phase 2 to
keep TS-produced corpora loadable. A future phase bumping it
without coordinating the TS writer at
`backend/src/sim/evaluateModelVsHeuristic.ts:367` would retroactively
break this corpus.

### 3b Chunks 1-4 — Pair-builder + DPO wiring + offline floor

- **Chunk 1 (`training/pair_builder.py`, 324 LOC stdlib)**: reads
  3a relabel rows, emits preference-pair JSONL in `runner-up` mode
  (winner=argmax visits, loser=second-highest,
  `sourceKind="mcts-relabel"`). 7 drop predicates counted in
  manifest; only `margin_below_floor` (default 0.05) fired against
  prod corpus. Aggregate: 11798 rows → **8093 pairs** (31.4%
  drop); search-vs-rule drop rate **~4× lower** (9.8%) than the
  other recipes — calibration evidence that played-MCTS sharpens
  relabel visit distributions. Margin histogram: 79% decisive
  (≥0.20). Smoke `pair_builder_smoke.py` wired as
  `npm run test:pair-builder`. Output:
  `runs/R16-TD-3a-prod-corpus/pairs/pairs{.jsonl,.manifest.json}`.
- **Chunks 2+3** (commits `5960cc3` / `1a071b9`): extended
  `pair_corpus.py` for the explicit pair schema and wired
  `card_ids_by_zone` / `action_card_idx` through `train_dpo.py`
  (v3 embedding branch now LIVE under DPO; grouped per-pair
  metrics across 5 axes).
- **Chunk 4 (`training/dpo_quality_eval.py`, 617 LOC)**: offline
  pair-quality gate per scoping § P2 step 5 ("no n=1000 gate until
  offline pair quality clears pre-registered floors"). Deterministic
  split (blake2b on `sourceEpisodeId:sourceStep`, 20% holdout, seed
  17), ranking metrics (top-1/2/3 + MRR, strict-> tie semantics
  over masked legal actions). Reference-baseline against 96-d
  production pin (1620 held-out rows): **`pair_accuracy` 0.6981 /
  `top1` 0.6512 / `top2` 0.8716 / `top3` 0.9321 / `MRR` 0.7921 /
  loss 0.6931 (= log 2, expected ref==self)**. JSON at
  `runs/R16-TD-3a-prod-corpus/baseline-eval.json` (gitignored).
  Trained DPO must improve on these before any closed-loop spend.
  Smoke wired as `npm run test:dpo-quality-eval`.
- **Compat**: `training/pad_checkpoint_to_v3.py` (numerically null
  pad) makes the pre-v3 96-d checkpoint usable as both ref and
  init for the v3-card-embedding-aware `train_dpo.py`.

### 3b Chunks 5a-5e — DPO probe series (collapsed)

Per-chunk landings + commits (dc68ac8 / c243fd8 / 7448b14 /
059f2a7) rolled to `r16-archive.md` § "R16-TD 3b chunks 5a-5e —
per-chunk landings". Headline numbers preserved in the
Cumulative Verdict table below.

### 3b — Cumulative Verdict

Four DPO probes against the 8093/13419-pair offline corpus all
land within the ±0.022 noise band of the baseline floor:

|  | corpus | epochs | β | lr | Δ pair_acc | Δ top1 |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| 5a | 8093 runner-up | 3 | 0.1 | 1e-5 | −0.0167 | −0.0321 |
| 5b | 8093 runner-up | 1 | 0.5 | 3e-6 | **+0.0043** | **+0.0006** |
| 5c-long | 8093 runner-up | 5 | 0.5 | 3e-6 | −0.0012 | −0.0117 |
| 5e | 13419 diversified | 1 | 0.5 | 3e-6 | +0.0031 | −0.0043 |

Best result is 5b at +0.0043 pair_acc / +0.0006 top1, well inside
noise. **DPO with this corpus + HP regime cannot extract a
statistically significant lift over the 96-d production
reference.** The offline gate (scoping § P2 step 5) holds — no
closed-loop n=1000 spend.

**Root-cause candidates + forward-line proposal** (4-candidate
list, original orchestrator recommendation): rolled to
`r16-archive.md` § "R16-TD 3b Cumulative Verdict — root-cause
candidates". Superseded by chunk 5f (candidate-3 falsified) +
chunk 5g (candidate-1 kill-test, see below).

### Cross-references

- Pre-registered design + corrected cost basis (not restated here):
  `docs/ai-research/scoping/r16-training-data-backlog-refinement.md`
  § "P1 - Rule-Bot-Covered States Relabeled By Rollout-Leaf MCTS"

### 3b — BC-on-relabel probe (chunk 5f) — FALSIFIES candidate-3

Direct soft-CE BC on the 11798-row 3a relabel corpus (the
cumulative-verdict candidate-3 test). HP: `--state-dim 110
--hidden-dim 64 --depth 2 --dropout 0.05 --epochs 3 --batch-size 32
--lr 3e-4 --split-by seed --seed 17 --data-mode bc`. Splits: 244
train seeds / 60 val seeds, 9379 train / 2419 val rows. Soft-CE
branch confirmed taken: `train_bc.py:490-498` keys on the batch
`policy_targets` tensor and every row in `relabel-all.jsonl`
carries `policyTargets`.

**Training trajectory** (`runs/R16-TD-3a-prod-corpus/bc-relabel-e3/train.log`):

| Epoch | train loss | val loss | val acc |
| ---: | ---: | ---: | ---: |
| 1 | 1.106 | 1.112 | 0.6945 |
| 2 | 1.074 | 1.106 | **0.7234 (peak)** |
| 3 | 1.059 | 1.105 | 0.7119 |

Mild overfit by epoch 3 (train still falling, val flat-to-rising).

**Eval against the same 1620-row pair floor** (seed 17, identical
holdout used for all DPO probes):

| Metric | Baseline (ref) | 5f (BC) | Δ vs base | Δ vs 5b (best DPO) |
| --- | ---: | ---: | ---: | ---: |
| `pair_accuracy` | 0.6981 | **0.6802** | **−0.0179** | −0.0223 |
| `ranking_top1` | 0.6512 | 0.6198 | **−0.0314** | −0.0321 |
| `top2` | 0.8716 | 0.8630 | −0.0086 | −0.0098 |
| `top3` | 0.9321 | 0.9278 | −0.0043 | −0.0037 |
| `MRR` | 0.7921 | 0.7748 | −0.0173 | −0.0177 |

All five metrics regress; `pair_accuracy` lands **outside the ±0.022
noise band**. BC is meaningfully worse than the untrained 96-d ref,
not flat.

Per-action-kind val accuracy (from `manifest.json`): routine actions
are high (`playBasic` 0.938 / `attachEnergy` 0.856 / `useAbility`
0.861 / `evolve` 0.831 / `attack` 0.805) but the high-branching
contested heads drag the headline — `pass` 0.632, `playTrainer`
0.591, `retreatAttack` 0.484. These are exactly the decisions the
relabel corpus was supposed to teach.

**Verdict**: candidate-3 (soft-CE BC fixes the DPO ceiling) is
**FALSIFIED**. The objective is not the constraint — BC actively
regresses the policy below the untrained ref on the same pair floor,
not just flat. By elimination, candidate-1 (relabel-MCTS too weak vs
ref) is **strengthened**: the corpus's `policyTargets` distribution
actively misleads training relative to the untrained reference,
consistent with rollout-leaf 100-sim MCTS producing labels worse
than the ref's own policy on contested states.

**Forward line** (user direction call): per the queue loop_note
CEILING PATH refinement, the live options are (a) re-relabel with
stronger MCTS (400+ sims / value-head leaf) — direct test of
candidate-1; or (b) W6 HP-sweep (CEILING PATH step A, cheap-first)
on the already-landed `r12_orchestrator.py` recipe-fix flags. The
3b workstream has hit its productive limit absent a direction call.

#### Candidate-1 kill-test (chunk 5g) — labels diverge decisively

Read-only label-divergence audit on the 11798-row 3a relabel
corpus: forward the untrained 96-d ref
(`runs/R13-W6-phase-d/iter-2/checkpoint.pt`) on each row's
`observation`, softmax over `legalActions`, compare to the row's
MCTS `policyTargets`. No MCTS, no training. Output:
`runs/R16-TD-3a-prod-corpus/candidate1-killtest/`
(`metrics.json`, `metrics-bc-val-slice.json`,
`metrics-confidence.json`, `notes.md`).

**Verdict: Case B — labels confidently wrong, not insufficient.**
Stronger-MCTS re-relabel is no longer the cheap-first
candidate-1 test.

Full corpus (n=11,798): top-1 agreement relabel-vs-ref **0.6338**;
mean KL(relabel‖ref) **0.331 nats**, KL(ref‖relabel) **0.766 nats**
(asymmetric — relabel near-zeros ref's plausible choices); mean
JSD **0.0739 nats**; mean entropy relabel 0.766 vs ref 0.876 →
relabel is *more* committed than ref (not budget-starved).

Per-action-kind agreement (binned on relabel argmax kind) — the
three heads where BC val acc cratered are exactly the three with
worst kill-test agreement:

| kind | n | agree | KL(rel‖ref) |
| --- | ---: | ---: | ---: |
| playTrainer | 4732 | **0.499** | 0.415 |
| pass | 2353 | **0.479** | 0.424 |
| retreatAttack | 140 | **0.464** | 0.327 |
| attachEnergy | 2572 | 0.848 | 0.241 |
| evolve | 700 | 0.863 | 0.117 |
| playBasic | 554 | 0.986 | 0.095 |
| attack | 471 | 0.781 | 0.225 |
| useAbility | 276 | 0.822 | 0.152 |

Disagreement confidence (n=4,320 disagreeing rows): relabel
max-prob when disagreeing mean **0.579**, median 0.54, **p90
0.91** — hyper-confident different picks. Relabel prob on ref's
choice: mean 0.227, p10 0.03 — ref's pick often near-zero under
relabel. 27% of all rows (n=3,121) have relabel confidence ≥0.8;
in that bucket ref/relabel still only agree 70% of the time.

Per-action causal-clean correlation: BC val acc cratered at
`pass` 0.581 / `playTrainer` 0.562 / `retreatAttack` 0.111
(from `bc-relabel-e3/trained-eval.json`) — exactly the three
heads with worst kill-test agreement. Labels are confidently
wrong, not insufficient.

**Sharpened diagnostic ladder** (supersedes the
cumulative-verdict 4-candidate forward-line proposal):

1. **Audit relabel value-eval on contested states** (cheapest).
   50–100 rows from `pass`/`playTrainer`/`retreatAttack` where
   relabel max-prob ≥ 0.8 and disagrees with ref. Re-run 100-sim
   MCTS but log per-leaf rollout returns. If returns are
   noise-dominated across CRN-paired rollouts → leaf eval is the
   bug, not sim count. Inspect rollout-policy choice (rule-bot
   vs random); rule-bot mirror may systematically devalue `pass`
   / `retreatAttack`.
2. **Argmax-flip test at 400 sims** on 200 contested-confident
   rows. >30% flip → sim count is load-bearing, candidate-1
   lives. >80% sticky → bug is upstream of sim count.
3. Skip more-sims BC entirely unless step 2 shows argmax
   instability.
