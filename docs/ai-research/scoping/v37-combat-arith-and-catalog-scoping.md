...
- **Date:** 2026-05-26
- **Status:** **LANDED-SOFT-SHIP 2026-05-26** — A1 fired against v3.6 cap128 cont-iter2 baseline (0.5880). n=10k tight-gate result: **wl_lower = 0.5870, wilson95 [0.5870, 0.6063]**, player=0.5556, opp=0.6105. Δ vs v3.6 = **−0.0010** (within Wilson noise). LIFT band (≥0.6040) NOT crossed. SOFT SHIP per §5 rule 2 — v3.7 ships as new schema baseline retiring v3.6. **Bet outcome §9 FLAT — the "argues back against §4m" hypothesis is falsified**: introducing qualitatively new representational classes (weakness arithmetic, coin-flip indicators, conditional-bonus flags, tool/ability effect-kind one-hot) also fails to crack the ~0.59 ceiling. The schema-axis exhaustion claim from §4m hardens decisively — the verdict now holds not just for bit-density rotations (v3.6) but also for qualitatively-new feature classes (v3.7). Per §8 rule 2: next pivot is off-axis (per-side-asymmetry-probe primary; set-attention Slice 2 re-baseline; history features R7.b.4). Canonical writeup: `progress/r110.md §4n`. Manifest: `runs/R16-P1-v37-cap128-iter6-tight-gate/gate.manifest.json`. Implementation chain: commits `d76c046` (scope), `674682f` (Phase A enums+classifiers+catalog smoke), `28caab4` (Phase B builders+dispatch), `ac5c463` (Phase C ckpt expander+parity smoke).
...
**`LANDED-SOFT-SHIP 2026-05-26`** — A1 n=10k wl_lower=**0.5870** vs v3.6 A0 0.5880 (Δ=−0.0010, within Wilson noise). LIFT band (≥0.6040) NOT crossed. SOFT SHIP — v3.7 retires v3.6 as schema-axis candidate-of-record. §9 bet FLAT: qualitatively-new representational classes do not crack the ~0.59 ceiling. Schema-axis exhaustion verdict from §4m HARDENS — now covers both bit-density rotations (v3.6) and qualitatively-new feature classes (v3.7). Implementation chain (4 commits): `d76c046` scope/queue → `674682f` Phase A → `28caab4` Phase B → `ac5c463` Phase C → A1 fire+gate this commit. Canonical verdict writeup: `progress/r110.md §4n`. Per §8 rule 2, next pivot is off-axis (`per-side-asymmetry-probe` primary, set-attention Slice 2 re-baseline, history features tertiary).
