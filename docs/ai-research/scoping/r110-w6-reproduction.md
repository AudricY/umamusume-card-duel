...
**Verdict: READY end-to-end. No code blocker. The only delta vs W6 is config (no warm-start), not missing capability.**
...
- `training/train_bc.py` — `--data-mode mcts-distill` soft-visit branch unchanged; `feature_schema_metadata()` reports current 110-d/v3.0 + card vocab. Card-embedding inputs handled.
