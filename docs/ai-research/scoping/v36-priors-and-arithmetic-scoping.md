...
- **Date:** 2026-05-25
- **Status:** **IMPLEMENTED-AWAITING-A1 2026-05-25** — Phases 1-6 LANDED. Obs-contract extension + Python builder + Rust mirror + schema dispatch + ckpt expander + Python↔Rust byte-parity smoke all green. A1 fire pending user approval. Implementation chain at §11. **Bet context updated:** v3.5 progressed to `§4k ceiling-confirmed at recipe scale` per queue entry update — schema axis declared EXHAUSTED at hidden_dim=64/depth=2/KL=0.05. v3.6 is therefore a schema-axis push AGAINST a "schema exhausted" verdict, with the explicit user bet that *info-density-per-bit* (not raw bit count) is the unmeasured lever.
