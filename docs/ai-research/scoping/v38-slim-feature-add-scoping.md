...
**`DRAFT — USER-GATED 2026-05-26`** — Slim two-axis additive scope after §13 evidence audit reduced original triple-axis draft by ~70%. STATE_DIM_V3_8=304 (+8 bits per-bench ETA + gust-swing). ACTION_FEATURE_COUNT=52, schema v4 (+4 cross-bit slots). Trunk UNCHANGED h128/d2. Single-arm fire (A0 reused, A1 bundle); ~30 min wall. Awaiting user approval to begin Phase A. Canonical destination: this doc. Queue entry id reserved: `v38-slim-feature-add`.
