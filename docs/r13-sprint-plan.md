...
- **W1 (parallelism):** DONE. `--workers N` flag on `sim:eval-gate` and `sim:mcts-selfplay`. Parent partitions tasks, forks N child processes, aggregates results via IPC (gate) or shard-concat (selfplay). Also pinned ORT to single-threaded execution in `serve_onnx.py` to remove float-reduction non-determinism under concurrent /predict callers. Speedups of 1.83x and 1.44x observed on tiny smokes; real-scale runs should approach Nx. Engine `Math.random()` leaks across async awaits (task #34) mean serial-vs-parallel bit-exact replay is not guaranteed; aggregate signal is preserved.
- **W2 (latency dials):** DONE. Added `--mcts-adaptive-ratio` and `--mcts-adaptive-min-sims` to `runMcts`. When the root's top-action visit count exceeds `(secondMax + 1) * ratio`, the sim loop halts early. Smoke shows **3.86x speedup** at ratio=2.0 on a 4-game value-head gate — exactly the regime where R12's "62% of decisions are 1-action forced" diagnostic predicted. K=1 rollouts are already CLI-tunable. Batched /predict was intentionally NOT implemented (rollouts dominate leaf cost ~30:1 in rollout-leaf mode; not worth the refactor).
- **W3 (value-head retrain):** **PARTIAL** verdict (2026-05-11). 50 rollout-leaf selfplay games → 25-epoch frozen-trunk MSE retrain → 100-game gate at `--mcts-leaf value-head`. WR 0.44 / Wilson lower **0.347** vs R4's 0.30 baseline (+4.7pp Wilson). Materially better than R4 but doesn't clear the 0.40 GO threshold. The rollout-mean target reduces value-head variance, but not enough on its own. Per the decision rules, PARTIAL → run Phase D (W6) with the W3-retrained checkpoint as warm-start so visit-count distillation can compound. Verdict at `runs/R13-value-retrain-experiment/verdict.json`.
- **W4 (MCTS ladder):** DONE (plumbing + ladder runner). `--opponent-selection mcts` puts the non-model side on its own MCTS (with optional `--opponent-mcts-*` overrides). Smoke validates the path runs both sides through MCTS. The full 3-pairing ladder is intentionally deferred to land alongside W3's verdict so we can compare value-head vs rollout-leaf at fixed sims.
- **W5 (UI integration):** plumbing DONE. Backend `POST /ai/decide` wraps `runMcts` and returns `{actionIndex, decisionMs, simulationsRun, haltedEarly}`. 0/1-legal-action positions short-circuit so the UI's existing rule-bot advance step can run. Frontend `mctsClient.ts` is a fetch+AbortController wrapper with 5s default timeout. Smoke validates endpoint reachability + input validation. Full UI hook (settings toggle, opponent-loop dispatch) is staged as a separate change because applying a chosen action requires either porting `advanceModeledTurnStep` to the frontend engine or adding a thin wrapper.
- **W6 (Phase D real run):** **DONE — compounding observed (2026-05-11).** 2 iterations × {60 rollout-leaf selfplay → 20-epoch distill (mcts-distill, KL anchor 0.05) → 120-game gate}, init from the W3-retrained checkpoint, 4 workers. Iter-0 Wilson lower 0.536 (WR 0.625); iter-1 Wilson lower **0.570** (WR 0.658). +3.4pp iter-on-iter Wilson improvement → compounding. Sides symmetric on iter-1: player 0.667 / opponent 0.65 (R-WILD side gap closed). Zero heuristic fallbacks. Iter-1 checkpoint at `runs/R13-W6-phase-d/iter-1/checkpoint.pt` is the new best model.
- **W7 headline (n=100):** DONE. Independent gate (seeds 600000+) at iter-1 checkpoint, rollout-leaf MCTS, 100 sims. WR 0.67, Wilson95 [0.573, 0.754]. Side asymmetry returned at this seed range (player 0.55 / opponent 0.78 — Wilson CIs overlap so plausibly seed noise, but worth a quick repro). Production headline: **Wilson lower ≥ 0.57 vs rule-bot at n≥100**, comfortably past the R12 baseline (0.556 at n=200) and the original 0.40 GO bar.
