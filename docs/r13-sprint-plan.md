...
- **W3 (value-head retrain):** plumbing DONE; experiment IN FLIGHT. `ValueTargetDataset` swaps the value target from backfilled game-z to `rootValue` (mean of K rule-bot CRN rollouts at the root, lifted out of `runMcts.diagnostics`). `r13_value_retrain.py` freezes the trunk + policy head and trains only the value head with weighted MSE. The dispositive driver script (`r13_value_retrain_experiment.py`) runs the full pipeline (rollout-leaf selfplay → retrain → gate at `--mcts-leaf value-head`). 50-game selfplay + 25-epoch retrain + 100-game gate is running in background; verdict will write to `runs/R13-value-retrain-experiment/verdict.json`.
...
- **W6 (Phase D real run):** waiting on W3 verdict.
