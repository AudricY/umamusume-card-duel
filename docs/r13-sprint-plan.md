...
- **W6 (Phase D real run):** **DONE — compounding observed (2026-05-11).** 2 iterations × {60 rollout-leaf selfplay → 20-epoch distill (mcts-distill, KL anchor 0.05) → 120-game gate}, init from the W3-retrained checkpoint, 4 workers. Iter-0 Wilson lower 0.536 (WR 0.625); iter-1 Wilson lower **0.570** (WR 0.658). +3.4pp iter-on-iter Wilson improvement → compounding. Sides symmetric: player 0.667 / opponent 0.65 (R-WILD side gap closed). Zero heuristic fallbacks. Iter-1 checkpoint at `runs/R13-W6-phase-d/iter-1/checkpoint.pt` is the new best model.
