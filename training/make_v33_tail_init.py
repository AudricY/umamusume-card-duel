"""v33-additive-tail: build a v3.3 additive-tail warm-start from a v3.1 source.

v3.3 widens the state-features vector from 164 → 167 by appending 3 opp-side
flag bits (usedSupporter / usedRetreat / usedStadium) at the end. The only
weight that changes shape is `state_encoder.0.weight` (the first Linear of
the state-features encoder MLP), which grows from `(hidden_dim, 164)` to
`(hidden_dim, 167)`. The 3 new columns are zero-initialized — combined with
the v3.3 builder emitting 0.0 for both own and opp default-false flags, a
v3.1-trained checkpoint produces bit-identical iter-0 outputs through the
v3.3 graph on observations where the opp-side flags are false (the
default).

Methodology (mirror of `training/make_v32_slot_token_init.py`):

  1. Read the v3.1 source ckpt (e.g. `runs/R16-P1-v31-ablation/loop/iter-3/checkpoint.pt`).
  2. Build a fresh v3.3 `CandidatePolicyNet` (state_dim=167) and discover
  ...
  3. Construct the merged state_dict:
     - For every key OTHER than `state_encoder.0.weight`, copy verbatim
       from the source (the v3.3 graph preserves them all).
     - For `state_encoder.0.weight`: take the v3.1 source weight of shape
       `(hidden, 164)` and pad with 3 zero columns on the right to
       `(hidden, 167)`. The bias is unchanged.
  4. Strict-load the merged state into a fresh v3.3 model — must accept
     every key, no missing, no unexpected.
  5. Sanity check: feed a fixed deterministic observation through both
     v3.1 (source) and v3.3 (merged) models; assert max|delta| < 1e-6 on
     logits and value outputs (the zero-init residual contract).

Refuses to operate on a non-v3.1 source: v3.0 / v3.2 ckpts have different
state_encoder.0 shapes (110-d input not 164-d) and would silently produce a
broken merge.
...
def build(source: Path, dest: Path) -> dict[str, Any]:
    """Build a v3.3 init checkpoint from a v3.1 source checkpoint.
    ...
    if "model_config" not in payload:
        raise SystemExit(
            ...
            f"v3.1 training checkpoint emitted by `train_bc.py`."
        )
    ...
    if source_state_dim != STATE_DIM_V3_1:
        raise SystemExit(
            f"v33-additive-tail builder refuses non-v3.1 source: "
            f"`{source}` has state_dim={source_state_dim}, expected "
            f"{STATE_DIM_V3_1}. v3.0 (110) and v3.2 (110+slots) have "
            f"different state_encoder.0 shapes and would silently corrupt "
            f"the merge. Use a v3.1 ckpt (e.g. "
            f"runs/R16-P1-v31-ablation/loop/iter-3/checkpoint.pt)."
        )

    if WIDENED_KEY not in source_state:
        raise SystemExit(
            f"source state_dict missing `{WIDENED_KEY}`. Cannot perform the "
            f"v3.1 → v3.3 state-vector widening without it."
        )
    source_weight = source_state[WIDENED_KEY]
    if source_weight.shape[-1] != STATE_DIM_V3_1:
        raise SystemExit(
            f"`{WIDENED_KEY}` has shape {tuple(source_weight.shape)}; "
            f"expected last dim {STATE_DIM_V3_1}. Source ckpt schema "
            f"disagrees with model_config.state_dim."
        )

    # Build the v3.3 target ModelConfig: copy source config, set state_dim
    # to 167. uses_uma_slot_tokens stays whatever the source had (this
    # builder is the state-vector widening axis only — the slot-token
    # axis is independent).
    target_config_dict = dict(source_config_dict)
    target_config_dict["state_dim"] = STATE_DIM_V3_3
    target_config = ModelConfig.from_dict(target_config_dict)
    if target_config.state_dim != STATE_DIM_V3_3:
        raise SystemExit(
            f"internal: target ModelConfig.state_dim did not propagate "
            f"{STATE_DIM_V3_3} after `from_dict` — ModelConfig schema drifted."
        )

    # Construct a fresh v3.3 model to learn the new shape of the widened
    ...
    # Widening: (hidden, 164) → (hidden, 167) by appending 3 zero columns.
    zeros = torch.zeros(
        (source_weight.shape[0], STATE_DIM_V3_3 - STATE_DIM_V3_1),
    ...
    # Strict-load into a fresh v3.3 model.
    ...
    if missing or unexpected:
        raise SystemExit(
            ...
            f"match the v3.3 model schema exactly."
        )

    # Bit-identical iter-0 contract: feed a fixed observation through
    # both the v3.1 source and the v3.3 merged model. v3.1 sees the first
    # 164 dims; v3.3 sees those 164 + 3 zero opp-flag bits. Because the
    # appended Linear columns are zero, the contribution is exactly 0.0
    # and outputs must be bit-identical (within fp tolerance).
    verify_v31 = CandidatePolicyNet(ModelConfig.from_dict(source_config_dict))
    missing, unexpected = verify_v31.load_state_dict(source_state, strict=True)
    if missing or unexpected:
        raise SystemExit(
            f"v3.1 source state_dict failed strict load: missing={list(missing)} "
    ...
    # don't care about realism; what matters is comparing v3.1 vs v3.3
    # on the SAME state_features with the appended bits = 0.
    state_v31 = torch.randn(batch, STATE_DIM_V3_1)
    state_v33 = torch.cat(
        [state_v31, torch.zeros(batch, STATE_DIM_V3_3 - STATE_DIM_V3_1)], dim=-1
    )
    ...
    # comparing is the v3.1 vs v3.3 trunk on identical aux inputs.
    from uma_ai.model import (  # noqa: E402
        ACTION_PAIR_FANOUT,
        NUM_ZONES,
    )
    ...
    if target_config.uses_uma_slot_tokens:
        from uma_ai.features import UMA_SLOT_COUNT, UMA_SLOT_FEATURE_DIM  # noqa: E402
        extra_kwargs["uma_slot_card_ids"] = torch.zeros(batch, UMA_SLOT_COUNT, dtype=torch.long)
        extra_kwargs["uma_slot_features"] = torch.zeros(batch, UMA_SLOT_COUNT, UMA_SLOT_FEATURE_DIM)

    with torch.no_grad():
        out_v31 = verify_v31(
            state_features=state_v31,
    ...
    logits_v31 = _to_tensor(out_v31)
    logits_v33 = _to_tensor(out_v33)
    if logits_v31 is None or logits_v33 is None:
        raise SystemExit(
            f"could not extract logits from model output for verification "
            f"(types: v31={type(out_v31).__name__}, v33={type(out_v33).__name__})"
        )
    delta_logits = float((logits_v31 - logits_v33).abs().max().item())
    if delta_logits > 1e-5:
        raise SystemExit(
            f"v3.1 ↔ v3.3 iter-0 parity FAILED: max|Δlogits|={delta_logits:.3e} > 1e-5. "
    ...
    # (export_onnx, serve_onnx, training resume) tag this as v3.3.
    feature_schema = dict(payload.get("feature_schema") or {})
    feature_schema["state_dim"] = STATE_DIM_V3_3
    feature_schema["state_feature_schema_version"] = STATE_FEATURE_SCHEMA_VERSION_V3_3

    provenance = {
        "method": "v3.3 opp-side flag additive-tail warm-start",
        ...
        "new_columns_zero_init": STATE_DIM_V3_3 - STATE_DIM_V3_1,
        "all_other_params": "copied verbatim",
        "delta_logits_at_init": delta_logits,
        "delta_logits_tolerance": 1e-5,
        "builder": "training/make_v33_tail_init.py",
    }

    # Warm-start init only (matches `make_v32_slot_token_init.py`): drop
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        dest="source",
        required=True,
        help="Path to the v3.1 source checkpoint to warm-start from "
             "(e.g. `runs/R16-P1-v31-ablation/loop/iter-3/checkpoint.pt`). "
             "Refuses non-v3.1 sources (state_dim != 164).",
    )
    parser.add_argument(
        "--output",
        "--dest",
        dest="dest",
        required=True,
        help="Path to write the v3.3 init checkpoint.",
    )
    ...
    print(
        f"v3.3 init built from {source}, output={dest}, "
