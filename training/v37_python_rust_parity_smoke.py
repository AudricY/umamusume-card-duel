"""v3.7 Python↔Rust featurizer parity smoke.

Loads each `<name>.json` fixture under
`engine-rs/crates/engine/tests/fixtures/v37_parity/`, runs
`observation_to_features_v3_7` on the parsed dict, and compares the
result against the matching `<name>.f32.bin` emitted by the Rust
integration test
(`engine-rs/crates/engine/tests/v37_python_parity_fixtures.rs`).

Parity contract (two-tier — the v3.7 ADDITIVE contract is the gate)
--------------------------------------------------------------------
Gate A — STRICT BIT IDENTITY on the v3.7-new tail [246:296]
  All 50 v3.7 tail bits are pure 0.0/1.0 booleans (weakness lethal,
  coin-flip indicators, conditional-bonus presence, ETA, paralysis,
  tool kind, ability kind). ANY mismatch fails the smoke loudly.

Gate B — 4-ULP TOLERANCE on the inherited v3.6 head [0:246]
  The v3.0 builder pre-existing slot 42 = `hash_average(handCardIds)`
  sums in Python f64 then downcasts to f32, while Rust sums entirely
  in f32. This is a 1-ULP artifact of the original v3.0 builder
  (max_abs ≈ 3e-8) and was never part of the v3.7 contract. We
  TOLERATE up to 4 ULPs (~5e-7 at unit scale) on the head and REPORT
  per-slot drift so a real divergence (>4 ULP) still fails. This
  mirrors v3.6's Gate B exactly.

Regenerate fixtures (after a deliberate Rust featurizer change):
    cargo test -p engine --test v37_python_parity_fixtures -- --ignored emit

Run the smoke:
    python training/v37_python_rust_parity_smoke.py
...
# v3.7-new contract slots (frozen — mirror `_V37_*` offsets in features.py).
_V37_NEW_TAIL_BAND = (STATE_DIM_V3_6, STATE_DIM_V3_7)  # [246:296], 50 bits.
...
def _load_rust_vector(bin_path: Path) -> np.ndarray:
    ...
    if len(raw) != STATE_DIM_V3_7 * 4:
        raise SystemExit(
            f"FAIL: {bin_path.name} byte length {len(raw)} != "
            f"{STATE_DIM_V3_7 * 4} (STATE_DIM_V3_7={STATE_DIM_V3_7} × 4 bytes)"
        )
...
def _strict_band(name: str, lo: int, hi: int, rust: np.ndarray, py: np.ndarray
    ...
    if bad.size == 0:
        return []
    lines = [
        f"FAIL[A]: {name} v3.7-new tail [{lo}:{hi}] diverges at "
        f"{bad.size} slot(s):"
    ]
...
def _head_drift(name: str, rust: np.ndarray, py: np.ndarray
    ...
    """Report per-slot drift on the inherited head slots [0:246].
    Returns (failure lines for slots > tolerance, all-drift dict)."""
...
def main() -> int:
    if not _FIXTURE_DIR.exists():
        print(
            ...
            "--test v37_python_parity_fixtures -- --ignored emit",
            file=sys.stderr,
        )
        return 1
    ...
    for json_path in json_paths:
        ...
        try:
            py_vec = observation_to_features_v3_7(observation)
        except Exception as exc:  # noqa: BLE001 — surface any exception
            failures.append(
                f"FAIL: {name}: observation_to_features_v3_7 raised "
        ...
        if py_vec.dtype != np.float32:
            ...
            continue
        if py_vec.shape != (STATE_DIM_V3_7,):
            failures.append(
        ...
        if gate_a_fail or gate_b_fail:
            ...
            print(f"PASS: {name} — v3.7 tail bit-identical{head_note}")
            pass_count += 1

    summary_lines = [
        "",
        "--- v37_python_rust_parity_smoke summary ---",
        f"  fixtures:   {len(json_paths)}",
        f"  passed:     {pass_count}",
        f"  failed:     {len(failures)}",
        f"  state_dim:  {STATE_DIM_V3_7}",
        f"  Gate A:     STRICT bit-identity on slots "
        f"[{_V37_NEW_TAIL_BAND[0]}:{_V37_NEW_TAIL_BAND[1]}] (v3.7-new contract)",
        f"  Gate B:     head [0:{STATE_DIM_V3_6}] ULP-bounded, "
    ...
    if failures:
        ...
        return 1
    print("OVERALL: PASS (v3.7 contract tail byte-identical; "
