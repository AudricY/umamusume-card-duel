...
def _row_to_sample(example: dict, *, min_actions: int, ablations: set[str] | None) -> MctsSelfPlaySample | None:
    ...
    *,
    ...
    with Path(path).open("r", encoding="utf8") as fh:
        for line_number, line in enumerate(fh, start=1):
            ...
            sample = _row_to_sample(example, min_actions=min_actions, ablations=ablations)
...
class ValueTargetDataset(MctsSelfPlayDataset):
    ...
    def __init__(
        ...
        *,
        ...
        self.samples = list(_load_value_target_samples(self.path, min_actions=min_actions, ablations=self.ablations))
        if not self.samples:
